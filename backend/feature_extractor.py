...
class WhisperFeatureExtractor:
    def __init__(self, layer_indices=None):
        ...
        self.layer_weights = nn.Parameter(
            torch.ones(len(layer_indices)), requires_grad=False
        )
    ...
    def _load_audio(audio_path: str):
        """Load audio via wave module (WAV) or bundled ffmpeg fallback."""
        import wave, struct
        if audio_path.lower().endswith(".wav"):
            try:
                with wave.open(audio_path, "rb") as wf:
                    ...
                    samples = np.array(
                        struct.unpack(f"<{n_frames * channels}h", raw), dtype=np.float32
                    )
                    samples /= 32768.0
                    waveform = torch.from_numpy(samples.reshape(channels, -1))
                    return waveform, sr
            except Exception:
                pass
        ...
        try:
            ...
            return WhisperFeatureExtractor._load_audio(tmp.name)
        finally:
            os.unlink(tmp.name)
