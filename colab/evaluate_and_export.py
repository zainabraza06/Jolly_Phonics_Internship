...
# =============================================================================
# The exported inference.py used to be generated from a ~360-line string
# literal here, re-declaring the whole model and feature stack. That code
# now lives once in backend/phonics/, so the bundle ships data only.
...
=============================================================
  ...
  (runtime code lives in backend/phonics/ — this bundle is data only)

USAGE IN APP BACKEND:
  from phonics import PhonemeGesturePredictor
  predictor = PhonemeGesturePredictor()
  analysis = predictor.analyse(video_path)
  scores   = predictor.score(analysis, label, group="child")
=============================================================
""")
