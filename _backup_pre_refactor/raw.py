...
if __name__ == "__main__":
    ...
    print(f"  {fusion_ckpt_path}") our best model has changed now it is fusion one your tasks 1. remove the unnecesary redundant code and seconldy for website repalce the existing code wiht the new one and thridly the socirng menchisim is currently scoring too low the distance between featres apprach need some refinement and lastly appintegraiotn folder has everhting properly organize the code in standard way and format .. professional look  and ru nthe applicatoin 